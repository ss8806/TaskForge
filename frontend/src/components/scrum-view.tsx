...
export function ScrumView({ projectId, onTaskClick }: ScrumViewProps) {
  ...
  return (
    <div className="flex flex-col gap-8">
      {/* Sprints Sections */}
      <div className="space-y-4">
        <h3 className="text-muted-foreground text-sm font-medium uppercase tracking-wider px-2">
          アクティブ & 将来のスプリント
        </h3>
        {sprints?.length === 0 ? (
          ...
          sprints?.map((sprint) => (
            <SprintSection
              ...
              onMoveToBacklog={(taskId) =>
                updateTaskMutation.mutate({ taskId, data: { sprint_id: null } })
              }
            />
          ))
        )}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        <Badge
          variant="outline"
          className={cn("text-[8px] h-4 px-1", priorityColors[task.priority])}
        >
