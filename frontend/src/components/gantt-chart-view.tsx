...
export function GanttChartView({ projectId }: GanttChartViewProps) {
  ...
  if (isLoading) {
    return (
      <div className="h-[400px] bg-secondary/30 rounded-2xl border border-border animate-pulse" />
    );
  }
    ...
    <div className="flex flex-col gap-4 bg-secondary/20 rounded-3xl border border-border overflow-hidden">
        ...
        <div className="w-48 shrink-0 border-r border-border bg-secondary/10 z-10 shadow-xl shadow-black/5">
          ...
          <div className="overflow-hidden">
            {tasks?.map((task) => (
              <div
                key={task.id}
                className="h-10 border-b border-border/50 flex items-center px-4 hover:bg-secondary/30 transition-colors"
              >
            ...
            <div className="relative">
              ...
              <div className="relative z-10">
                {tasks?.map((task) => (
                  <div
                    key={task.id}
                    className="h-10 border-b border-border/30 flex items-center relative"
                  >
                    <TaskBar
                      task={task}
      ...
      <div className="p-3 bg-secondary/40 border-t border-border flex justify-center gap-6 text-[10px] text-muted-foreground italic">
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-primary" /> 予定あり
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-secondary" /> 未設定
        </div>
      </div>
    </div>
  );
