        ...
        <div className="text-center space-y-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 animate-in fade-in slide-in-from-bottom-16 duration-1000 delay-500">
            <Link href="/register">
              <Button
                size="lg"
                className="h-14 px-8 bg-primary text-primary-foreground hover:bg-primary/90 text-lg font-bold rounded-2xl shadow-xl shadow-primary/20 group"
              >
                今すぐ始める
            ...
            <Button
              size="lg"
              variant="outline"
              className="h-14 px-8 border-border text-foreground hover:bg-secondary text-lg font-bold rounded-2xl"
            >
              ドキュメントを読む
            </Button>
          </div>
          ...
          {[
            {
              icon: Brain,
              title: "AI自動分解",
              desc: "自然言語の要件からエピック、タスク、工数見積もりを瞬時に生成。",
            },
            {
              icon: Layout,
              title: "動的ビュー",
              desc: "カンバン、スクラム、ガントチャートを1クリックでシームレスに切替。",
            },
            {
              icon: Shield,
              title: "エンタープライズ品質",
              desc: "洗練されたUI/UXと堅牢なセキュリティでチームの生産性を最大化。",
            },
          ].map((feature, i) => (
            <div
              key={i}
              className="group p-8 rounded-3xl bg-secondary/50 border border-border hover:border-primary/30 transition-all"
            >
