...
export default function ProjectPage() {
  ...
  const [taskForm, setTaskForm] = useState<{
    ...
    description: string;
    sprint_id?: number;
  }>({ title: "", description: "", sprint_id: undefined });
  ...
  const aiDecomposeMutation = useMutation({
    mutationFn: () =>
      aiApi.decompose(
        projectId,
        aiPrompt,
        aiSprintId === "none" ? undefined : Number(aiSprintId)
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["projects", projectId, "tasks"],
          ...
          <div className="flex items-center gap-3">
            ...
            <Dialog open={isAiModalOpen} onOpenChange={setIsAiModalOpen}>
              <DialogTrigger asChild>
                <Button
                  size="sm"
                  className="bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 font-semibold h-9 rounded-xl"
                >
            ...
            <Dialog>
              <DialogTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="bg-transparent border-border text-muted-foreground hover:text-foreground flex items-center gap-2"
                >
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <TabsList className="bg-secondary/50 border border-border h-10 p-1">
              <TabsTrigger
                value="kanban"
                className="data-[state=active]:bg-background data-[state=active]:text-foreground h-8 flex gap-2"
              >
          ...
          {activeTab === "kanban" && (
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <option value="backlog" className="bg-background">
                  バックログのみ
                </option>
          ...
          {isTasksLoading ? (
            <div className="grid grid-cols-3 gap-6">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-[400px] bg-secondary/50 rounded-2xl border border-border animate-pulse"
                />
              ))}
            </div>
          ) : activeTab === "kanban" ? (
