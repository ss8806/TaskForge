...
class AIDecompositionStatusResponse(BaseModel):
    ...
    status: str = Field(description="ジョブステータス")
    message: str = Field(description="メッセージ")
    result: dict | None = Field(None, description="結果（完了時）")
    meta: dict | None = Field(None, description="メタ情報")
...
def start_decomposition(
    ...
    task = decompose_tasks_async.delay(
        project_id=project_id, user_requirement=body.prompt, sprint_id=body.sprint_id
    )
...
@router.get("/ai/jobs/{job_id}", response_model=AIDecompositionStatusResponse)
